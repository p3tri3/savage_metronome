...
# TECHNICAL DECISIONS
- **rodio 0.21.1 API**: `OutputStreamBuilder::open_default_stream()` returns `OutputStream`; `Mixer` (from `stream.mixer().clone()`) is the shared handle passed to audio thread. `Sink::connect_new(&mixer)` creates sinks.
...
- Windows: `.ico` embedded into PE via `embed-resource` (build.rs + windows.rc)
- macOS: `.icns` bundled via `cargo-bundle` metadata in Cargo.toml

Build is clean (no errors, no warnings) as of 2026-02-17.
